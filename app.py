...
st.set_page_config(
    page_title="Accident Severity Prediction App", page_icon="🚧", layout="wide"
)
...
st.markdown(
    "<h1 style='text-align: center;'>Accident Severity Prediction App 🚧</h1>",
...
def main():
    with st.form("road_traffic_severity_form"):
        ...
        No_vehicles = st.slider(
            "Number of vehicles involved:", 1, 7, value=0, format="%d"
        )
        ...
        Education = st.selectbox(
            "Education of driver:", options=options_education_level
        )
        service_vehicle = st.selectbox(
            "Service year of vehicle:", options=options_services_year
        )
    ...
    if submit:
        ...
        st.markdown(
            """Reach out to me on: [Twitter](https://twitter.com/avikumart_) |
...
st.markdown(
    "Please find GitHub repository link of project: [Click Here](https://github.com/avikumart/Road-Traffic-Severity-Classification-Project)"
)
